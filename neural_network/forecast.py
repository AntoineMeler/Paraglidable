...
class Forecast:
	...
	@classmethod
	def __generate_tiler_argument_file(cls, outputFilename, predictionFilename, tilesDir, tilerCacheDir, bordersFilename, minZoom,
	...
	def main(self):
		...
		for last_forecast_time in last_forecast_times[0:4]:
			...
			for days in range(self.nb_days):
				...
				self.compute_prediction_file_cells(# predictions
												   self.__compute_cells_forecasts(self.models_directory, self.problem_formulation, meteo_matrix),
												   # other params
				...
				if self.render_tiles:
					self.__generate_tiler_argument_file(self.tiler_arguments_filename,
												        self.prediction_filename_for_tiler,
												        tiles_dir_this_day,
